...
const Instauto = async (db, browser, options) => {
  ...
  async function likeCurrentUserImagesPageCode({
    ...
    for (const image of images) {
      ...
      const section = Array.from(dialog.querySelectorAll('section')).find(
        (s) => s.querySelectorAll('*[aria-label="Like"]')[0]
   ...
   * @function
   * @param {Object} options
   * @param {string[]} options.usersToFollowFollowersOf
   * @param {number} [options.maxFollowsTotal]
   * @param {boolean} options.skipPrivate
   * @param {boolean} [options.enableFollow]
   * @param {boolean} [options.enableLikeImages]
   * @param {number} [options.likeImagesMin]
   * @param {number} [options.likeImagesMax]
   * @returns {Promise<void>}
  ...
  async function processUsersFollowers({
    ...
    const maxFollowsPerUser = enableFollow && usersToFollowFollowersOfSliced.length > 0
  ...
  async function doesUserFollowMe(username) {
    try {
      ...
      return users.some(
        (user) => String(user.pk) === String(myUserId) || user.username === myUsername,
